...
func TestGetInvalidPort(t *testing.T) {
	...
	if err.Error() != `sshconfig: strconv.ParseUint: parsing "notanumber": invalid syntax` {
...
Host kevinburke.sshconfig.test.example.com
    Port 4567
`)
...
func TestInclude(t *testing.T) {
	...
	val := us.Get("kevinburke.sshconfig.test.example.com", "Port")
	if val != "4567" {
...
func TestIncludeRecursive(t *testing.T) {
	...
	val, err := us.GetStrict("kevinburke.sshconfig.test.example.com", "Port")
...
func TestMatchUnsupported(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "sshconfig: Match directive parsing is unsupported") {
